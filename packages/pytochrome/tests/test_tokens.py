...
def test_cycle_length(mode: ModeTokens) -> None:
    assert len(mode.aes.cycle) == 8


@pytest.mark.parametrize("mode", [DARK, LIGHT])
def test_cycle_valid_hex(mode: ModeTokens) -> None:
    for color in mode.aes.cycle:
        assert _is_hex(color), f"Invalid hex in cycle: {color!r}"


@pytest.mark.parametrize("mode", [DARK, LIGHT])
def test_text_colors_valid_hex(mode: ModeTokens) -> None:
    assert _is_hex(mode.theme.text_primary), (
        f"text_primary: {mode.theme.text_primary!r}"
    )
    assert _is_hex(mode.theme.text_secondary), (
        f"text_secondary: {mode.theme.text_secondary!r}"
    )
    assert _is_hex(mode.theme.accent), f"accent: {mode.theme.accent!r}"


@pytest.mark.parametrize("mode", [DARK, LIGHT])
def test_rgba_tokens_valid(mode: ModeTokens) -> None:
